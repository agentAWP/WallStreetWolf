...
def marketData():
    ...
    finVizScreener = finVizMarketScreener()
    return render_template("marketData.html",sectors = sectors,marketMovement=marketMovement,finVizScreener=finVizScreener)
...
def companyFinancials():
    return render_template("companyFinancials.html")

@app.route("/companyFinancialsResult/",methods = ["GET","POST"])
def companyFinancialsResult():
    tickers = [request.form["symbol"]]
    stockStatements = {}
    for ticker in tickers:
        stockStatements[ticker] = stockanalysisFundamentals(ticker)
    return render_template("companyFinancialsResult.html",stockStatements=stockStatements,tickers=tickers)
